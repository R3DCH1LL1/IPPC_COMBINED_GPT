...
async function fetchPackedData(){
  ...
  catch(e){return await readJsonResponse(await fetch(`/questions.packed.json?v=${DATA_VERSION}`,{cache:'no-store'}));}
}
...
function MockApp({onBack=()=>{},onNotes=()=>{},theme:sharedTheme,toggleTheme:sharedToggleTheme=()=>{}}){
  ...
  useEffect(()=>{const onResize=()=>setIsMobileViewport(window.innerWidth<=760);onResize();window.addEventListener('resize',onResize);window.addEventListener('orientationchange',onResize);return()=>{window.removeEventListener('resize',onResize);window.removeEventListener('orientationchange',onResize);}},[]);
  useEffect(()=>{
    if(screen!=='test' || !isMobileViewport){
      setMobileFitScale(1);
      setMobileFitHeight(null);
      return;
    }
    let raf1=0, raf2=0;
    const recalc=()=>{
      ...
      const available=Math.max(320,Math.floor(window.innerHeight-top-8));
      setMobileFitHeight(available);
      raf2=window.requestAnimationFrame(()=>{
        const fullHeight=content.scrollHeight||1;
        const nextScale=Math.min(1,Math.max(0.72,(available-4)/fullHeight));
        setMobileFitScale(Number(nextScale.toFixed(3)));
      });
    };
    raf1=window.requestAnimationFrame(()=>window.requestAnimationFrame(recalc));
    return()=>{window.cancelAnimationFrame(raf1);window.cancelAnimationFrame(raf2);};
  },[screen,isMobileViewport,idx,showAns,showStudy,selected,navOpen,theme,compact,qs.length]);
  ...
  if(err)return <div className="center error-state"><div className="empty-card"><h1>Could not load app</h1><p>{err}</p></div></div>;
  if(!bank.length)return <div className="center"><div className="empty-card"><div className="spin"/><p>Loading compact question bank…</p></div></div>;

  return <div className={`app-shell theme-${theme} screen-${screen} ${compact?'compact-mode':''}`}>
    <header className="topbar"><button className="brand-btn" onClick={()=>setScreen('home')}><span className="brand-mark">IPPC</span><span className="brand-sub">Mock Suite</span></button><div className="topbar-actions"><nav className="topnav" aria-label="Main navigation"><button className={screen==='home'?'nav-btn active-nav':'nav-btn'} onClick={()=>setScreen('home')}>Home</button><button className="nav-btn" onClick={onBack}>Study Suite Menu</button><button className="nav-btn" onClick={onNotes}>Notes Page</button><button className={screen==='summary'?'nav-btn active-nav':'nav-btn'} onClick={()=>setScreen('summary')}>Summary Breakdown</button><button className={screen==='bank'?'nav-btn active-nav':'nav-btn'} onClick={()=>setScreen('bank')}>Question Bank</button><button className={screen==='history'||screen==='results'?'nav-btn active-nav':'nav-btn'} onClick={()=>setScreen('history')}>Progress</button></nav><button className="theme-toggle-btn" onClick={()=>setSettingsOpen(true)} aria-label="Open settings">⚙️ Settings</button></div></header>
    ...
    {screen==='test'&&current&&<main className="page page-test page-test-drawer">
      ...
      <section className={`surface question-panel drawer-test-panel ${isMobileViewport?'mobile-fit-shell':''}`}>
        <div ref={fitStageRef} className={isMobileViewport?'mobile-fit-stage':''} style={isMobileViewport&&mobileFitHeight?{height:mobileFitHeight}:undefined}>
        <div ref={fitContentRef} className={isMobileViewport?'mobile-fit-content':''} style={isMobileViewport?{transform:`scale(${mobileFitScale})`}:undefined}>
...
function CombinedLanding({onEnter,theme,toggleTheme}){
  ...
  <main className="portal-shell">
    <header className="portal-menu-topbar">
      ...
      <button className="theme-toggle-btn" onClick={toggleTheme} aria-label="Toggle colour theme">
...
function NotesPortal({onBack,onMock,theme}){
  return <div className={`notes-portal-shell theme-${theme}`}>
    <header className="portal-topbar">
      ...
      <div className="portal-topbar-actions">
        <button className="secondary-btn" onClick={onBack}>Study Suite Menu</button>
        <button className="secondary-btn" onClick={onMock}>Mock Test Page</button>
        <button className="primary-btn" onClick={()=>window.open('/notes/index.html','_blank')}>Open full page</button>
      </div>
    </header>
...
function CombinedApp(){
  ...
  if(view==='notes') return <NotesPortal onBack={()=>go('landing')} onMock={()=>go('mock')} theme={theme}/>;
