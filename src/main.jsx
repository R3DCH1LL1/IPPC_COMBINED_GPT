...
const REPORT_PDF_PATH='/IPPC_Question_Bank_Analytical_Report_V70_Deep_Audited_Final.pdf#toolbar=0&navpanes=0&scrollbar=1';
...
function CombinedLanding({onEnter,theme,toggleTheme}){
  ...
  <main className="portal-shell">
    ...
    <section className="portal-choice-grid">
      ...
      <button className="portal-card portal-surface portal-resource-card" onClick={()=>onEnter('report')}>
        <div className="portal-card-icon">📊</div>
        <div>
          <span className="eyebrow muted">Review quality</span>
          <h2>V70 Analytical Report</h2>
      ...
      <button className="portal-card portal-surface portal-resource-card portal-download-card" onClick={()=>onEnter('printable')}>
        <div className="portal-card-icon">🖨️</div>
        <div>
          <span className="eyebrow muted">Printable PDF</span>
          <h2>Printable Study Notes</h2>
          <p>View the audited black-and-white printable notes inside the suite, with a download button available on the notes page.</p>
        </div>
        <span className="portal-arrow">View →</span>
      </button>
    </section>
  ...
  return <div className={`notes-portal-shell theme-${theme}`}>
    <header className="portal-topbar">
      <button className="brand-btn" onClick={onBack}>
        <span className="brand-mark">IPPC</span>
        <span className="brand-sub">V70 Analytical Report</span>
      </button>
      <div className="portal-topbar-actions">
        ...
        <button className="theme-toggle-btn notes-theme-btn" onClick={()=>setSettingsOpen(true)} aria-label="Open report settings"><span aria-hidden="true">⚙︎</span></button>
      </div>
    </header>
    {settingsOpen&&<div className="settings-overlay notes-settings-shell">
      ...
      <aside className="settings-drawer">
        ...
        <div className="settings-body">
          <button className="settings-row" onClick={toggleTheme}><span>Theme</span><strong>{theme==='dark'?'Dark':'Light'}</strong></button>
          <button className="settings-row" onClick={()=>setSettingsOpen(false)}><span>Close settings</span><strong>Done</strong></button>
        </div>
      </aside>
    </div>}
    <iframe className="notes-frame report-frame" title="IPPC V70 Analytical Report" src={REPORT_PDF_PATH} />
  </div>
}
  ...
  return <div className={`notes-portal-shell theme-${theme}`}>
    <header className="portal-topbar">
      ...
      <div className="portal-topbar-actions">
        ...
        <button className="theme-toggle-btn notes-theme-btn" onClick={()=>setSettingsOpen(true)} aria-label="Open printable notes settings"><span aria-hidden="true">⚙︎</span></button>
      </div>
    </header>
    {settingsOpen&&<div className="settings-overlay notes-settings-shell">
      <button className="settings-backdrop" onClick={()=>setSettingsOpen(false)} aria-label="Close settings"/>
    ...
    <iframe className="notes-frame report-frame printable-pdf-frame" title="IPPC Printable Study Notes" src={PRINTABLE_NOTES_VIEW_PATH} />
  </div>
}
