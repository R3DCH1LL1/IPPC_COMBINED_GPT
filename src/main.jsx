    ...
    {screen==='test'&&current&&<main className="page page-test page-test-drawer">
      ...
      <section className="surface question-panel drawer-test-panel">
        <div className="question-toolbar sticky-test-header">
          ...
          <div className="test-toolbar-side"><div className="toolbar-pills"><Pill>Set {current.set}</Pill><Pill>{current.topic}</Pill><Pill>CLO {current.clo}</Pill><Pill>{difficulty(current)}</Pill>{running&&<span className="timer-pill">{fmt(seconds)}</span>}</div><button className="danger-btn" onClick={finishSession}>Finish session</button></div>
        </div>
        ...
        <div className="button-row split-mobile test-action-bar">
          <button className="secondary-btn mobile-nav-button" onClick={()=>setNavOpen(true)}>Navigator</button>
          <button className="secondary-btn" onClick={()=>jump(idx-1)} disabled={idx===0}>← Previous</button>
          <button className="secondary-btn" onClick={()=>setShowAns(!showAns)}>{showAns?'Hide answer':'Show answer'}</button>
          <button className="secondary-btn" onClick={()=>setFlags(f=>({...f,[current.id]:!f[current.id]}))}>{flags[current.id]?'Unflag':'Flag'}</button>
          <button className="primary-btn next-primary" onClick={()=>idx===qs.length-1?finishSession():jump(idx+1)}>{idx===qs.length-1?'Submit results':'Next →'}</button>
        </div>
      </section>
    </main>}
